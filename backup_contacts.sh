#!/bin/bash
#
# Batchjob zum Backup der Contacts
...
#
# Autor: Jörg Schultze-Lutter, 2026
#
#
...
#<dict>
#  <key>Label</key>
#  <string>com.jsl.backup_contacts</string>
#
#  <key>ProgramArguments</key>
#  <array>
#    <string>/Users/jsl/cronjobs/backup_contacts.sh</string>
#  </array>
#
...
#  <string>/tmp/backup-contacts-launchd.out</string>
#
#  <key>StandardErrorPath</key>
#  <string>/tmp/backup-contacts-launchd.err</string>
#
...
#
# speichern als ~/Library/LaunchAgents/com.jsl.backup_contacts.plist
#
# Installieren: launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.jsl.backup_contacts.plist
#
# Manuell ausführen: launchctl kickstart -k gui/$(id -u)/com.jsl.backup_contacts
#
# Deinstallieren: launchctl bootout gui/$(id -u) ~/Library/LaunchAgents/com.jsl.backup_contacts.plist
#
...
main() {
  ...
  if [ ! -f "$MACPORTS_PATH/7zz" ]; then
	  ...
	  if [ "$EUID" -ne 0 ]; then
		  osascript -e 'display notification "Cannot create backup - 7z command not found!" with title "Contacts Backup"' > /dev/null 2>&1
	  fi
	  exit 1
  fi
  ...
  local schluessel="PASSWORD_CONTACTS_BACKUPS"
  local MEINPASSWORT

  MEINPASSWORT=$(get_password "$schluessel" "$PWFILE")
  case $? in
    0) : ;;
    1) logger "Password key \"$schluessel\" not found in $PWFILE."; osascript -e 'display notification "Key not found in password file" with title "Contacts Backup"' > /dev/null 2>&1;exit 1 ;;
    2) # unsichere Rechte oder Stat-Fehler
       logger "Insecure password file $PWFILE or other error has occurred."; osascript -e 'display notification "Insecure password file or other error" with title "Contacts Backup"' > /dev/null 2>&1;exit 1 ;;
    *) exit 1 ;;
  esac
  ...
  $MACPORTS_PATH/7zz a -t7z -mx=9 -bd -bb0 -y $BACKUP_DIR/$PROJECT_NAME-$DATE.7z "$CONTACTS_SRC_DIR" >/dev/null </dev/null
  ...
  $MACPORTS_PATH/7zz a -t7z -mx=9 -mhe=on -p"$MEINPASSWORT" -bd -bb0 -y $CLOUD_BACKUP_TEMP/$PROJECT_NAME/$PROJECT_NAME-$DATE.7z "$CONTACTS_SRC_DIR" >/dev/null </dev/null
  ...
  logger Have created Contacts backup on date $DATE. Backup retention time is set to $BACKUP_RETENTION files.

  #sofern nicht root, dann per osascript Notification an den User erstellen
  if [ "$EUID" -ne 0 ]; then
          osascript -e 'display notification "Have created Contacts database backup" with title "Cron job report"' > /dev/null 2>&1
  fi
...
# convert to POSIX-compliant syntax for AppleScript usage
tmpdir_as_path="$(osascript -e 'on run argv
    return (POSIX file (item 1 of argv)) as text
end run' "$tmpdir")"

# run the export
osascript /Users/jsl/cronjobs/ContactsExport.scpt "$tmpdir_as_path" "$EXPORT_DIR"

main $tmpdir/$EXPORT_DIR "$@"
rm -r $tmpdir
